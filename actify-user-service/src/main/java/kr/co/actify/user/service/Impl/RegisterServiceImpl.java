...
import kr.co.actify.user.model.dto.register.RegisterDTO;
import kr.co.actify.user.model.dto.register.RegisterReq;
...
import kr.co.actify.user.service.RegisterService;
import kr.co.actify.user.service.UserQueryService;
import kr.co.actify.user.util.AESUtil;
import kr.co.actify.user.util.BCryptUtil;
import kr.co.actify.user.util.EmailTemplateProvider;
...
public class RegisterServiceImpl implements RegisterService {
    ...
    public RegisterDTO signup(RegisterReq registerReq) {
        ...
        UsersInformation usersInformation = UsersInformation.builder()
                ...
                .nameEnc(aesUtil.encrypt(registerReq.getName()))
                .mailEnc(aesUtil.encrypt(registerReq.getMail()))
                .mailHash(hmacSha256HashUtil.mailHash(registerReq.getMail()))
                .phoneNumberEnc(aesUtil.encrypt(registerReq.getPhoneNumber()))
                .phoneNumberHash(hmacSha256HashUtil.phoneNumberHash(registerReq.getPhoneNumber()))
                .birthEnc(aesUtil.encrypt(registerReq.getBirth()))
                .profileImageUrl("")
        ...
        EmailMessage emailMessage = EmailMessage.builder()
                .to(registerReq.getMail())
                .subject("[Actify] 회원가입 인증번호 안내해 드립니다.")
        ...
        registerDTO.setMail(registerReq.getMail());
